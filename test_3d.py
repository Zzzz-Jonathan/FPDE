...
def my_plot(_x, _y, _z, _n, name=None):
    X, Y, Z = np.mgrid[-3.5:5:100j, 0:5:50j, -2.5:2.5:100j]

    n_new = griddata((_x, _y, _z), _n, (X, Y, Z), method='nearest')

    if normalize:
        exec('global norm; norm = %s_norm' % name[-1])

    print(n_new[:, 25, :].shape)

    plt.imshow(n_new[:, 25, :], cmap=plt.get_cmap('hot'), norm=norm)
    plt.colorbar()
