...
from parameter import noisy_3d_rate, dataset, BATCH_3d, device, gradients, LOSS
from parameter import Re_4d as RE
...
def loss_pde(nn, data_inp):
    [t, x, y, z] = torch.split(data_inp, 1, dim=1)
...
def loss_les(nn, data_inp, dx=0.05, dy=0.05, dz=0.05):
    ...
    for xs in xxs:
        for ys in yys:
            for zs in zzs:
                txys.append(torch.cat([t, xs, ys, zs], dim=1))
...
def icbc_save():
    ...
    ic_data = np.array([[t, x_min, y, z] for t in np.arange(0, 2.3, 0.2)
                        for y in np.arange(-0.5, 5.6, 0.5)
                        for z in np.arange(-3, 3.1, 0.5)])

    bc_data_c = np.array([[t, (np.cos(i) / 2) - 1, y, np.sin(i) / 2] for t in np.arange(0, 2.3, 0.2)
                          for i in np.linspace(0, 2 * np.pi, 16)
                          for y in np.arange(-0.5, 5.6, 0.5)])

    bc_data_top = np.array([[t, x, y_max, z] for t in np.arange(0, 2.3, 0.2)
                            for x in np.arange(-4, 5.6, 0.5)
                            for z in np.arange(-3, 3.1, 0.5)])
    bc_data_btm = np.copy(bc_data_top)
    bc_data_btm[:, 2] = y_min

    bc_data_fot = np.array([[t, x, y, z_max] for t in np.arange(0, 2.3, 0.2)
                            for x in np.arange(-4, 5.6, 0.5)
                            for y in np.arange(-0.5, 5.6, 0.5)])
...
def data_save():
    ...
    for i, xyz in enumerate(x_y_z):
        ...
        t_x_y_z[:, i, 2] = xyz[1]
        t_x_y_z[:, i, 3] = xyz[2]
    t_x_y_z = t_x_y_z.reshape((T * N, 4))

    u, v, w, p = ori_label['u'], ori_label['v'], ori_label['w'], ori_label['p']
    u_v_w_p = np.concatenate((u, v, w, p), axis=2)
    T, N = u_v_w_p.shape[0], u_v_w_p.shape[1]
    u_v_w_p = u_v_w_p.reshape((T * N, 4))

    rng_state = np.random.get_state()
    np.random.shuffle(t_x_y_z)
    np.random.set_state(rng_state)
    np.random.shuffle(u_v_w_p)

    full_size = t_x_y_z.shape[0]
    train_size = int(full_size * (1 / 2))

    train_data = t_x_y_z[:train_size]
    train_label = u_v_w_p[:train_size]
...
dataset = dataset(torch.FloatTensor(t_data),
                  torch.FloatTensor(t_label))
dataloader_1 = DataLoader(dataset=dataset,
                          batch_size=BATCH_3d,
                          shuffle=True,
                          num_workers=8)
dataloader_2 = DataLoader(dataset=dataset,
